...
public:
	...
	{
		static bool is_moved = false;
		struct itemlist *p;
		if (false == check_capture(mx, my, msg))
			return;
		...
		else
		{
			switch (msg)
			{
				case WM_LBUTTONDOWN:
					...
					dolist (p, items){
						if (p->item->mouse_over(mx, my)){
							s_index = ((taskentry*)p->item)->m_index;
						}
					}
					break;
				case WM_MOUSEMOVE:
					...
					if (abs(mx-s_mx) < GetSystemMetrics(SM_CXDRAG))
						break;

					if (mPI->DnDTaskReorder == false)
						break;
					...
					dolist (p, items){
						if (p->item->mouse_over(mx, s_my)){
							taskentry* te = (taskentry*)p->item;
							if (te !=NULL){
								int index_to = te->m_index;
								if (index_to != s_index){
									int index_src  = s_index;
									int index_dest = index_to;
									if (mPI->currentOnly){
										int nIndex = 0;
										tasklist *tl;
										dolist(tl, GetTaskListPtr()){
											if (tl->wkspc != info.number){
												if (nIndex <= index_src)
													index_src++;
												if (nIndex <= index_dest)
													index_dest++;
											}
											nIndex++;
										}
									}
									SendMessage(BBhwnd, BB_MOVETASK, MAKELPARAM(index_src, index_dest) ,0);
									is_moved = true;
									mPI->NewTasklist();
									mPI->update_bar(2);
									s_mx = mx; s_my = my;
