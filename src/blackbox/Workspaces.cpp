...
//                     dbg_printf("BBWS_MOVEWINDOWTOWS hwnd=%x, lParam=%s", hwnd, lParam); 
                ...
                case BBWS_REMOVETASK:
                    tasklist* tl = (struct tasklist *)assoc(taskList, hwnd);
                    if (tl) RemoveTask(tl);
                    break;
            }
            break;
        ...
        default:
        {
            ...
            switch (msg)
            {
                ...
                case BB_WINDOWMINIMIZETOTRAY:
                    tasklist *p;
                    HWND hOwner = (HWND)GetWindow(hwnd, GW_OWNER);
                    if (hOwner == NULL) hOwner = hwnd;
                    dolist(p, taskList){
                        if (p->hwnd == hOwner || p->hwnd == hwnd){
                            WS_LowerWindow(hOwner);
                            MinimizeToTray(hOwner);
                        }
                    }
                    break;

                case BB_MOVETASK:
                    int src_index = LOWORD(wParam);
                    int dest_index = HIWORD(wParam);
                    move_node(&taskList, src_index+1, dest_index+1);
                    break;
            }
        }
    }
...
inline ST void window_set_pos(HWND hwnd, RECT rc)
{
...
//===========================================================================
ST void CALLBACK RestoreDefaultMaxPos(HWND hwnd, UINT uMsg, ULONG_PTR uData, LRESULT lResult){
    WINDOWPLACEMENT wp;
    wp.length = sizeof wp;
...
ST bool check_for_restore(HWND hwnd)
{
    ...
    // RestoreDefaultMaxPos : restore the default maxPos (necessary when it was V-max'd or H-max'd)
    SendMessageCallback(hwnd, WM_SYSCOMMAND, SC_RESTORE, 0, RestoreDefaultMaxPos, (DWORD)NULL);
    return true;
}
...
ST void MoveWindowToDirectWS(HWND hwnd, int ws_number, bool follow)
{
    ...
    if (follow) 
    {
//       dbg_printf("BBWS_MOVEWINDOW followMoved!");                        
      SwitchToWindow(hwnd);
    }
    else
    {
//       dbg_printf("BBWS_MOVEWINDOW not followMoved!");
      post_task_refresh();
      post_desk_refresh();
