...
*/
    ...
    struct string_node *startup_folder = NULL;

    char szPath[MAX_PATH]; int i;
    int len = sizeof(startuptable)/sizeof(startuptable[0]);
    for (i = 0; i < len; ++i){
        if (sh_getfolderpath(szPath, startuptable[i])){
            bool isExist = false;
            dolist(sl, startup_folder){ // ignore duplicated folder
                if (stricmp(sl->str, szPath) == 0)
                    isExist = true;
            }
            if (!isExist)
                append_string_node(&startup_folder, szPath);
        }
    }

    dolist(sl, startup_folder){
        RunFolderContents(sl->str);
    }

    freeall(&startup_folder);
