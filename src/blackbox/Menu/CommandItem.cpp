...
CommandItem::CommandItem(const char* pszCommand, const char* pszTitle, bool isChecked, const char* pszIcon)
    : MenuItem(pszTitle)
{
...
VolumeItem::VolumeItem(const char *pszTitle, const char *pszDllName, const char* pszIcon): CommandItem(NULL, pszTitle, false, pszIcon), VolumeControl(pszDllName)
{
...
{
    StyleItem *pSI = (m_bActive && Settings_menuVolumeHilite) ? &mStyle.MenuVolumeHilite : &mStyle.MenuVolume;

    // draw volume bar
    RECT rect; GetItemRect(&rect);
    _InflateRect(&rect, -2, -2);
    rect.right  -= (GetRectWidth(&rect))*(100-m_nVol)/100;
    if(GetRectWidth(&rect) > 0)
...
LRESULT CALLBACK StringItem::EditProc(HWND hText, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    switch(msg)
    {
        ...
        case WM_PAINT:
        {
            ...
            HGDIOBJ oldbuf = SelectObject(buf, CreateCompatibleBitmap(hdc, ps.rcPaint.right, ps.rcPaint.bottom));

            StyleItem *pSI = &mStyle.MenuFrame;
            MakeGradient(buf, ps.rcPaint, pSI->type, pSI->Color, pSI->ColorTo, pSI->interlaced, BEVEL_SUNKEN, BEVEL1, 0, 0, 0);
...
// SeparatorItem
SeparatorItem::SeparatorItem(): CommandItem(NULL, "", false, NULL)
{
    m_ItemID = MENUITEM_ID_CIInt;
    m_isNOP = MI_NOP_SEP;
}
...
{
    StyleItem *pSI = &mStyle.MenuSeparator;
    int margin = imax(MenuInfo.nItemIndent + Settings_separatorMargin, 0);

    RECT r; GetItemRect(&r);
    _InflateRect(&r, -margin, -2);

    BYTE SepType = !stricmp(Settings_separatorStyle, "toRight")      |
                   !stricmp(Settings_separatorStyle, "toLeft")  << 1 |
                   !stricmp(Settings_separatorStyle, "Outward") << 2;
    int hue;
